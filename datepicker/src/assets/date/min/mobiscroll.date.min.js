...
function onHide(prevAnim){var activeEl,value,type,focus=s.focusOnClose;$markup.remove();if($activeElm&&!prevAnim){setTimeout(function(){if(focus===undefined||focus===true){preventShow=true;activeEl=$activeElm[0];type=activeEl.type;value=activeEl.value;try{activeEl.type='button';}catch(ex){}
$activeElm.focus();activeEl.type=type;activeEl.value=value;}else if(focus){if(instances[$(focus).attr('id')]){ms.tapped=false;}
$(focus).focus();}},200);}
...
function show(beforeShow){if(!ms.tapped){if(beforeShow){beforeShow();}
...
function event(name,args){var ret;args.push(that);$.each([userdef,theme,preset,settings],function(i,v){if(v&&v[name]){ret=v[name].apply(el,args);}});return ret;}
that.position=function(check){var w,l,t,anchor,aw,ah,ap,at,al,arr,arrw,arrl,dh,scroll,sl,st,totalw=0,minw=0,css={},nw=Math.min($wnd[0].innerWidth||$wnd.innerWidth(),$persp.width()),nh=$wnd[0].innerHeight||$wnd.innerHeight();if((wndWidth===nw&&wndHeight===nh&&check)||preventPos){return;}
...
wndWidth=nw;wndHeight=nh;};that.attachShow=function($elm,beforeShow){elmList.push($elm);if(s.display!=='inline'){if(setReadOnly){$elm.on('mousedown.dw',function(ev){ev.preventDefault();});}
if(s.showOnFocus){$elm.on('focus.dw',function(){if(!preventShow){show(beforeShow);}});}
if(s.showOnTap){that.tap($elm,function(){show(beforeShow);});}}};that.select=function(){if(!isModal||that.hide(false,'set')!==false){that._fillValue();event('onSelect',[that._value]);}};that.cancel=function(){if(!isModal||that.hide(false,'cancel')!==false){event('onCancel',[that._value]);}};that.clear=function(){event('onClear',[$markup]);if(isModal&&!that.live){that.hide(false,'clear');}
that.setValue(null,true);};that.enable=function(){s.disabled=false;if(that._isInput){$elm.prop('disabled',false);}};that.disable=function(){s.disabled=true;if(that._isInput){$elm.prop('disabled',true);}};that.show=function(prevAnim,prevFocus){var html;if(s.disabled||that._isVisible){return;}
if(doAnim!==false){if(s.display=='top'){doAnim='slidedown';}
if(s.display=='bottom'){doAnim='slideup';}}
that._readValue();event('onBeforeShow',[]);html='<div lang="'+s.lang+'" class="mbsc-'+s.theme+' dw-'+s.display+' '+
(s.cssClass||'')+
...
(s.display==='bubble'?'<div class="dw-arrw"><div class="dw-arrw-i"><div class="dw-arr"></div></div></div>':'')+'<div class="dwwr"> <div class="user-defined"></div> '+'<div aria-live="assertive" class="dw-aria dw-hidden"></div>'+
 ...
 * @param {Object} $
 */
(function($){var themes=$.mobiscroll.themes,theme={display:"bottom",dateOrder:"MMdyy",rows:5,height:30,minWidth:50,headerText:false,showLabel:false,btnWidth:false,selectedLineHeight:true,selectedLineBorder:1,useShortLabels:true,deleteIcon:"backspace3",checkIcon:"ion-ios7-checkmark-empty",btnCalPrevClass:"mbsc-ic mbsc-ic-arrow-left5",btnCalNextClass:"mbsc-ic mbsc-ic-arrow-right5",btnPlusClass:"mbsc-ic mbsc-ic-arrow-down5",btnMinusClass:"mbsc-ic mbsc-ic-arrow-up5",onThemeLoad:function(lang,s){if(s.theme){s.theme=s.theme.replace("ios7","ios")}}};themes.ios=theme;themes.ios7=theme})($);
...
var datetimePlugChange = {
	...
	_this:this,
 	dateOpt:{
		...
		//minDate: new Date(),
		maxDate:new Date(2100,(new Date()).getMonth(),(new Date()).getDate()+7,23,59),
        dateFormat: 'yy-mm-dd', // 日期格式
        dateOrder: 'yymmdd', //面板中日期排列格式YYMMddDD
		stepMinute: 1, //设置分钟步长
		yearText:'年',
		monthText:'月',
		dayText:'日',
	...
	open:false,
	ele:false,
	ready:function( _this ,ele  ,opt ,callFun , _val ){
		datetimePlugChange.ele = ele ;
		datetimePlugChange.open = true ;
		datetimePlugChange.call = callFun ;
		datetimePlugChange._this = _this ;
		datetimePlugChange.date = _val|| "" ;
 		if($('#'+ele).length <1){
			var input = document.createElement('input');
				input.id=ele;
				input.type="hidden";
				document.body.appendChild(input);
				datetimePlugChange.hashchange = window.location.url ;
				$('#'+ele).on("change" , function(){
					datetimePlugChange.change(this.value)
				});
				if( ("onhashchange" in window) && ((typeof document.documentMode==="undefined") || document.documentMode==8)) {
					//浏览器支持onhashchange事件
					window.onhashchange = function(){
						datetimePlugChange.cancel();
					};
				} else {
					// 不支持则用定时器检测的办法
 					setInterval(function() {
						if(datetimePlugChange.hashchange!=window.location.url ){ datetimePlugChange.cancel(); }
					}, 150);
				}
		...
		setTimeout(function(){
			$('#'+ele).val(_val);
			$('#'+ele).mobiscroll(datetimePlugChange[ opt ] );
			$('#'+ele).click();
		})
	},
	call:function  (v){
		console.log( "已选中日期： "+ datetimePlugChange.date)
	},
	change:function  (v){
        if(v){
			datetimePlugChange.date = v ;
			datetimePlugChange.call(  v ) ;
		}else{
			datetimePlugChange.cancel() ;
		}
	},
	cancel:function(){
		$(".dwb1").click();
		datetimePlugChange.open = false ;
	}
}

export default datetimePlugChange ;
